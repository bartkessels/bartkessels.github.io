...
So now that we know what our oponents strategy is going to be, and what the expected outcome is going to be we need to think about how we can get our move based on the
expected outcome.

So in the following diagram there are three 
...
flowchart TB
  ...
  subgraph matchone["Match one"]
    ...
    subgraph matchoneo["Oponent strategy"]
      moos["Rock"]
    end

    subgraph matchoneown["Own strategy"]
        direction LR

      moeo["Expected outcome <b>Draw</b>"]
      getmoveone["GetMove()"]
      moveone["Rock"]
    ...
    playmatchone["Play match"]

    matchoneo --> playmatchone
    matchoneown --> playmatchone
  end

  subgraph matchtwo["Match two"]
    direction LR

    subgraph matchtwoo["Oponent strategy"]
      mtwhoo["Paper"]
    end

    subgraph matchtwoeo["Expected outcome"]
      mtwhoeo["Won"]
    end
    ...
    matchtwoo --> getmovetwo
    matchtwoeo --> getmovetwo
  ...
  subgraph matchthree["Match three"]
    ...
    subgraph matchthreeo["Oponent strategy"]
      mthreeeo["Paper"]
    end

    subgraph matchthreeeo["Expected outcome"]
      mteo["Won"]
    end

    getmovethree["GetMove()"]
    movethree["Scissors"]

    matchthreeo --> getmovethree
    matchthreeeo --> getmovethree

    getmovethree --> movethree
  end
...
the assignment. We only check for our own moves, because we don't get points for the move the oponent made.
