---
slug: create-an-android-app/set-up-view-models-with-jetpack-compose
...
author: Bart Kessels
date: 2024-02-28
image: /images/blog/android/create-an-application/set-up-view-models-with-jetpack-compose.png
...
<Intro>
    <Char>I</Char>n our [previous post](./20240221-set-up-jetpack-compose.mdx) we've built a basic Android application that can display _Hello World_. In some cases this might just be the app you're looking for, but generally speaking you'd want an app that executes some kind of logic.
</Intro>
...
<CodeBlock filename="app/build.gradle.kts">
...
<CodeBlock filename="src/main/kotlin/**/MainViewModel.kt">
...
In the [previous post](./20240221-set-up-jetpack-compose.mdx) we've created a view inside an Activity. Because we'd like to separate the logic from the UI, we'll be creating a new file where our UI will live. You can split this up into multiple files when you need to, but for now we'll be creating everything in the same file.

Create the file `MainView.kt` in the `app` module and give it the following contents.

<CodeBlock filename="src/main/kotlin/**/MainView.kt">
...
<CodeBlock filename="src/main/kotlin/**/MainActivity.kt">
...
![Our updated Android app](/images/blog/android/create-an-application/basic-android-application-view-model.png)
