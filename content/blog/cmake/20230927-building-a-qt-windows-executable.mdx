---
slug: building-a-qt-windows-executable
...
author: Bart Kessels
date: 2023-09-27
image: /images/blog/cmake/building-qt-windows-executable.png
...
For this tutorial we're going to use the setup from the [use CMake target in other folder](./20230913-use-cmake-target-in-another-folder.mdx) post from earlier.
...
Why this works, I suggest you read the [use CMake target in another folder](./20230913-use-cmake-target-in-another-folder.mdx) post from before.

Now we're going to implement the `CMakeLists.txt` file for the `packaging/windows` target with the following contents

<CodeBlock filename="packaging/windows/CMakeLists.txt">
```cmake
cmake_minimum_required(VERSION 3.15 FATAL_ERROR)
