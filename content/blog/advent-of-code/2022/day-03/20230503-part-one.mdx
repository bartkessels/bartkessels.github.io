---
slug: 2022/day-three/part-one
title: AoC 2022, Day three - Part One
description: Find the same items that are stored in the two compartments of each rucksack.
author: Bart Kessels
date: 2023-05-03
image: /images/blog/advent-of-code/2022/day-3.png
...
Now that we have the following data structure from our [previous post](./20230426-sanitizer.mdx), we can start by thinking about the business logic to retrieve the code that's inside all the rucksacks.
...
So now that we have our rucksacks with each compartment separated, we can think about how we want to setup our business logic. So to retrieve the code from all the rucksacks, we need to get the unique character that's present in both compartments.
...
<CodeBlock filename="aoc-2022/day3/src/main/kotlin/aoc/PartOne.kt">
```kotlin
class PartOne(
...
<CodeBlock filename="aoc-2022/day3/src/test/kotlin/aoc/PartOne.kt">
```kotlin
class PartOneTest {
    @Test
    fun testGetResult() {
