---
slug: 2022/day-five/part-two
title: AoC 2022, Day 5 - Part Two
...
Now that we have the following data structure from our [previous post](./20230607-sanitizer.mdx), we can start by thinking about the business logic to check how many clean up ranges are completely inside another clean up range.
...
But, you might say, this will result in exactly the same behavior as before. Well that's right! So there is one crucial step we need to take before pushing the value into the `stackToStoreItem` and that is the `reversed` method on the `items`.
...
Now we know what to change as opposed to part one, we can start implementing the `getResult` in the same way as [part one](./20230614-part-one.mdx). And use the `moveItem` as we've discussed in the design above.

<CodeBlock filename="aoc-2022/day5/src/main/kotlin/aoc/PartTwo.kt">
```kotlin
class PartTwo(
...
<CodeBlock filename="aoc-2022/day5/src/test/kotlin/aoc/PartTwo.kt">
```kotlin
class PartTwoTest {
    @Test
    fun testGetResult() {
