...
author: Bart Kessels
date: 2023-09-27
image: /images/blog/google-home/or-condition.png
...
<Intro>
    <Char>I</Char>n the [previous post](./20230920-and-condition.mdx) we've seen that we can use `and` conditions in automations. We also saw that there is an `or` condition operator. We'll in this post we're going to update our previous example and add an `or` condition.
</Intro>
...
In the [previous post](./20230920-and-condition.mdx) we saw that we can use the `or` operator to add multiple conditions that will allow the execution of the automation when at least one of them is true.
